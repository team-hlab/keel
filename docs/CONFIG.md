...
Outside those areas, writes **inside** `<root>` → **deny**; writes **outside** `<root>` → **pass** (defer). Reads of non-sensitive files → **allow**. Shell commands are parsed segment-by-segment (chains, pipes, subshells, `cd`, `git -C`, redirects).
