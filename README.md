...
- ✅ **Four features** — autopermit, branch-guard, secret-scan, session-banner — plus an opt-in **decision log** (`keel stats`). **Rust test suite** (unit + integration, incl. edge-case & fault-tolerance) **plus a dockerized e2e** that installs the real Claude/Codex/`agy` CLIs and exercises the full lifecycle; clippy/fmt clean.
...
Opt into a **decision log** (`"log": {"enabled": true}`) to record every call's verdict, then `keel stats` to see the mix + what's driving `ask` — zero overhead when off.
